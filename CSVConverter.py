...
#### Intsructions for Data Degitization
...
timeshift = r'0'
# comment
comment = r'Insert Comments Here'
# ignitionType
target = r'type'
type = r'with respect to'
# Define filetypDOI
...
if len(target)>0:
    xmld.write('<ignitionType ' + 'target= "' + target +'" type="' + type +
           '"/>' + "\n")
if len(fileDOI)>0:
...
for i in range(len(preferredKey)):
    xmld.write('<component><speciesLink' + " preferredKey=" + '"'+preferredKey[i] + '" CAS=' +CAS[i]+'"'+'/>')
    xmld.write('<amount'+ ' units=' +'"'+ str(units[i])+'"'+'>'+ str(amount[i])+'</amount>'+'</component>')
xmld.write('</property>'+ "\n")
if len(vals)>0:
    for i in range(len(vals)):
        xmld.write('<property' + " label=" + '"' + labels[i] + '"' +
                   " units=" + '"' + punits[i] + '"' +
                   " sourcetype=" + '"' + sourcetype + '">' +
                   " <value>" +  str(vals[i]) + '</value></property>')
